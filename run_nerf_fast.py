...
def render_rays(ray_batch,
    ...
    print(f"There is a fine network {network_fine is not None}")
...
def render(H, W, focal,
    ...
    ray_groups_o = np.zeros((d_factor, H//d_factor, W//d_factor, 3))
    ray_groups_d = np.zeros((d_factor, H//d_factor, W//d_factor, 3))
    
    # Shape: group# x group height x group width x 1
    near_groups = near * np.ones((d_factor, H//d_factor, W//d_factor, 1))
    far_groups = far * np.ones((d_factor, H//d_factor, W//d_factor, 1))

    for i in range(d_factor):
        for j in range(d_factor):
            xs = range(i, W, d_factor)
            ys = range(j, H, d_factor)
            width = len(xs)
            height = len(ys)
            ...
            ray_groups_o[i, :height, :width, :] = rays_g_o
            ray_groups_d[i, :height, :width, :] = rays_g_d
    ...
    ray_groups_o = tf.cast(tf.reshape(ray_groups_o, [d_factor, -1, 3]), dtype=tf.float32)
    ray_groups_d = tf.cast(tf.reshape(ray_groups_d, [d_factor, -1, 3]), dtype=tf.float32)
    near_groups = tf.cast(tf.reshape(near_groups, [d_factor, -1, 1]), dtype=tf.float32)
    far_groups = tf.cast(tf.reshape(far_groups, [d_factor, -1, 1]), dtype=tf.float32)
    ...
    # Render and reshape
    all_ret = batchify_rays(rays[0], chunk, **kwargs)
    for k in all_ret:
